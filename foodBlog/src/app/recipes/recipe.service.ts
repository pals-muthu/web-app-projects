...
export class RecipeService {
  ...
  temp_recipes: Recipe[];
  recipes: Recipe[] = [
    new Recipe(1, 'All American Cheeseburger', 'Assemble the ingredients',
    ...
    new Recipe(2, 'Thai inspired Ceviche', 'Assemble the ingredients',
  ...
  getRecipeById (id: number) {
    console.log("In the recipe service: ", id);
    this.temp_recipes =  this.recipes.filter((recipe) => {
      console.log("in filter: ", recipe.id, recipe.id == id);
      return recipe.id == id;
    });
    console.log("temp: ", this.temp_recipes);
    return this.temp_recipes[0];
  }
